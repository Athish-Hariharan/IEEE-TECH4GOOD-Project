...
def send_lora_command(command_string):
    ...
    try:
        ...
        print("[LoRa Script] Waiting for acknowledgement...")
        acknowledgement = ""
        start_time = time.time()
        timeout_seconds = 5  # Wait up to 5 seconds for acknowledgement
        ...
        while (time.time() - start_time) < timeout_seconds:
            if ser.in_waiting > 0:
                ...
                if '\n' in acknowledgement or '\r' in acknowledgement:
                    break
            time.sleep(0.1)  # Small delay to avoid busy waiting
        
        if acknowledgement:
            print(f"[LoRa Script] Acknowledgement received: {acknowledgement.strip()}")
        else:
            print(f"[LoRa Script] No acknowledgement received within {timeout_seconds} seconds.")
